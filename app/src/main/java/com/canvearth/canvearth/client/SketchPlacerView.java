 ...
 * visible window.
 */
public class SketchPlacerView extends AppCompatImageView implements OnScaleGestureListener {
    ...
    public SketchPlacerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SketchPlacerView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        scaleDetector = new ScaleGestureDetector(context, this);
        ScaleGestureDetectorCompat.setQuickScaleEnabled(scaleDetector, false);
        startScaleType = getScaleType();

        TypedArray values = context.obtainStyledAttributes(attrs, R.styleable.SketchPlacerView);

        zoomable = values.getBoolean(R.styleable.SketchPlacerView_sketch_zoomable, true);
        translatable = values.getBoolean(R.styleable.SketchPlacerView_sketch_translatable, true);
        animateOnReset = values.getBoolean(R.styleable.SketchPlacerView_sketch_animateOnReset, true);
        autoCenter = values.getBoolean(R.styleable.SketchPlacerView_sketch_autoCenter, true);
        restrictBounds = values.getBoolean(R.styleable.SketchPlacerView_sketch_restrictBounds, false);
        minScale = values.getFloat(R.styleable.SketchPlacerView_sketch_minScale, MIN_SCALE);
        maxScale = values.getFloat(R.styleable.SketchPlacerView_sketch_maxScale, MAX_SCALE);
        autoResetMode = AutoResetMode.Parser.fromInt(values.getInt(R.styleable.SketchPlacerView_sketch_autoResetMode, AutoResetMode.UNDER));
